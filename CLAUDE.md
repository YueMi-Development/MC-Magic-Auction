...
- **Winner Reveal Animation**: Runs when a winner is decided. Opens the reveal GUI showing all items as 1x1 black stained glass panes (`???`) and remains in this initial state for a **5-second delay** (100 ticks). Afterward, items are revealed one-by-one in-place to their actual fully-revealed icons. This is powered by two conditional GUI layers: `reveal_items` (priority 2, condition `finalI <= currentRevealStep`) and `reveal_placeholders` (priority 1, no condition).
...
- **Graph Visual Formatting**: Custom coloring rules on the bidding progress graph:
  ...
  - No BIN status/multiplier is shown on player heads.
